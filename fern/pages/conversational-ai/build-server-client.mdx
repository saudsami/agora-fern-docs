...
- **Backend**: Three HTTP endpoints:
  - <Badge intent="success">POST</Badge> `/api/token`: Issues RTC + RTM tokens for a given channel and UID
  - <Badge intent="success">POST</Badge> `/api/invite-agent`: Starts the agent using the Agent Server SDK
  - <Badge intent="success">POST</Badge> `/api/stop-conversation`: Stops the agent by ID
...
![](/assets/conversational-ai/server-sdk-flow.png)
...
<Callout intent="note" title="Coming soon">
...
Endpoint: <Badge intent="success">POST</Badge> `/api/token`
...
Endpoint: <Badge intent="success">POST</Badge> `/api/invite-agent`
...
Endpoint: <Badge intent="success">POST</Badge> `/api/stop-conversation`
...
```python title="server-python/main.py"
import os
import time
...
```ts title="hooks/useConvoAgent.ts"
'use client';
...
| Agent never speaks but `agentId` is returned | Conversational AI feature not enabled on the Agora project. | Run `agora project feature list`. If `convoai` is missing, rerun `agora project create --feature convoai` or enable it in Agora Console. |
